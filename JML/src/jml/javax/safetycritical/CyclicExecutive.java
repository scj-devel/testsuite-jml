...
public abstract class CyclicExecutive extends Mission
{
  ...
  // overrides the method in class Mission and is called in mission memory
  {
    final CyclicSchedule schedule = 
    	      getSchedule ((PeriodicEventHandler[]) mem.getHandlerPortal().eventHandlers);
    ...
    {      
      ...
      //devices.Console.println("CyclicEx.runExec while, n: " + n);
...
//  HSO: June 2013:  // HSO: commented out: only when JML specifying; The JML tools use Eclipse VM  
//	// temporarily; later use absolute time; see above
//	rtClock.getTime (now); 
//    // deltaTime = next - now
//    jml.vm.RealtimeClock.delayNative(next.subtract(now, deltaTime)); 
  }
}
