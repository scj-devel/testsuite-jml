 ...
 *             specification.
 */
final class CyclicScheduler extends Scheduler
{

	MissionSequencer seq;

	private ScjProcess current;
	CyclicSchedulerImpl cyclicSchedulerImpl;

	private static CyclicScheduler scheduler = new CyclicScheduler();
	...
	private CyclicScheduler() {
		this.cyclicSchedulerImpl = new CyclicSchedulerImpl();
		
		int[] sequencerStack = new int[Const.DEFAULT_STACK_SIZE];
		
		jml.vm.ClockInterruptHandler.initialize(this.cyclicSchedulerImpl, sequencerStack);
	}

	void start(MissionSequencer seq) {
		this.seq = seq;

		current = new ScjProcess(seq, new int[Const.DEFAULT_STACK_SIZE]);
