 ...
 */
@SCJAllowed
public abstract class ManagedEventHandler extends AsyncEventHandler 
  implements ManagedSchedulable 
{
    ...
    @*/
	 ...
	 /*@ 
       ...
       ensures this.priority == priority && this.release == release;
	...
	protected ManagedEventHandler(PriorityParameters priority, ReleaseParameters release, StorageParameters storage) {
		...
		this.privateMemory = new /*ManagedMemory*/ PrivateMemory((storage != null ? (int) storage.getBackingStoreSize() : Const.PRIVATE_MEM_SIZE));
