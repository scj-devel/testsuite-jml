...
public abstract class Mission 
{
  ...
  /*@ // implementation specification 
      requires true; 
      assignable \nothing;
      ensures \result == currMissSeq.currMission; 
    @*/
  @SCJAllowed
  ...
  /*@ // implementation specification 
      requires true; 
      assignable \nothing;
      ensures \result == currMissSeq.sequenceTerminationPending(); 
    @*/
  ...
  /*@ // implementation specification 
      requires true; 
      assignable \nothing;
      ensures \result == missionTerminate; 
    @*/
  ...
  {    
    ...
    for (int i = 0; i < handlerSet.noOfHandlers; i++)
    {       
      handlerSet.scjProcesses[i] = 
        new ScjProcess (handlerSet.eventHandlers[i], 
    		            new int[handlerSet.eventHandlers[i].default_stack_size]);
      handlerSet.scjProcesses[i].index = i;
      
      frame.addProcess	(handlerSet.scjProcesses[i]);      
    }
