...
class PriorityQueue
{
  ...
  private final int defalutCapacity = 10;

  public PriorityQueue ()
  {
    heapSize = 0;
    tree = new int[defalutCapacity + 1];
  ...
  {
	  //devices.Console.println("PriorityQueue.getScjProcess:  treeIdx = " + treeIdx);
	  if (processIdx == -999)
	  {
		  devices.Console.println("PriorityQueue.getScjProcess -999");
		  return null;
	  }  
	  if (processIdx == -2)
	  {
		  //devices.Console.println("PriorityQueue.getScjProcess -2");
		  return MissionSequencer.missSeqProcess;
	  }
	  if (processIdx == -1)
	  {
		  //devices.Console.println("PriorityQueue.getScjProcess -1");
		  return ScjProcess.idleProcess;
	  }
	  //devices.Console.println("PriorityQueue.getScjProcess " + processIdx );
	  return HandlerSet.getHandlerSet().scjProcesses[processIdx];
  }
