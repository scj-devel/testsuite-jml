...
final class ScjProcess implements Comparable<ScjProcess> 
{
	...
	private class HandleAsyncEventRunner implements Runnable
	{
		@Override
		public void run() {
	...
	private HandleAsyncEventRunner runner;
	...
	ScjProcess(ManagedEventHandler handler, int[] stack) {
		...
		this.runner = new HandleAsyncEventRunner();
		
		this.process = new jml.vm.Process(new Runnable() {
			public void run() {
				try {
					target.privateMemory.enter(runner);
				} catch (Exception e) {
				  System.out.println("ScjProcess: exception -> " + e);
	 ...
	 * processes are done after growing priorities.
	 */
	 ...
	 * 
	 * First implementation: Idle process is a periodic handler with "infinite"
	 * period.
	 */
	...
	public static ScjProcess createIdleProcess() {
		if (idleProcess == null) {
			...
			idleProcess = new ScjProcess(new PeriodicEventHandler(new PriorityParameters(Priorities.MIN_PRIORITY), new PeriodicParameters(
					new RelativeTime(Clock.getRealtimeClock()),// start (0,0)
					Const.INFINITE_TIME), // period
					new StorageParameters(Const.PRIVATE_MEM_SIZE, null) // size
																		// of
																		// private
																		// mem
					) {
						...
						private void yield() {
							...
							while (true) {
								count++;
								if (count == 100000) {
									// devices.Console.println("--> idleProcess, count: "
									// + count);
									count = 0;
								}
							}
		...
		// devices.Console.println("ScjProcess.idleProcess created: " +
		// idleProcess);
